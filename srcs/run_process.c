...
/*   Updated: 2016/09/04 12:21:41 by arnovan-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_arg_code(t_uchar encode, t_arg_code *arg_code, int opcode)
{
	...
	arg_code->total =
		get_arg_len(arg_code->arg1, g_op_tab[opcode - 1].is_index) +
...
void		run_process(t_env *env, t_process *pro)
{
	...
	if (pro->cycle_to_next == 0)
	{
		...
		{
			pro->carry = (*g_function[opcode])(env, arg_code, pro);
			if (pro->carry == 0 || opcode != 9)
